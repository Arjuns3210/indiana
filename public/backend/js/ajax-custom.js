...
function addLoading(){
     $('.card-content').append('<div class="overlay">'+
    ...
    if (can == 0) {
        $.ajax({
            ...
            beforeSend: function () {
                ...
                //uses only mipo form screen 
                if ($('.reject-btn').length) {
...
function submitMipoEnquiry(form_id, form_method) {
    var form = $('#' + form_id);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (response['success'] == 0) {
                    ...
                    $('#enquiry_id').val(data.id).trigger('change');
...
function submitModalForm(form_id, form_method, errorOverlay = '') {
    ...
    $.ajax({
        ...
        error: function () {
            var errorMsg = 'Something is wrong, please try again later.';
            //removeLoading();
             $(form).find('.form-error').html('<span class="text-danger">*' + errorMsg + '</span>');
                    setTimeout(function () {
                        $(form).find('.form-error').empty();
                    }, 3000);
            $(".btn-success").attr("disabled", false);
...
async function categoryStatusForSale(sales_id) {
    await $.ajax({
        ...
        success: function (result) {
            ...
            categoryStatusValues = Object.values(result.category_status_count).map(Number);
        }
    });
...
async function engineerStatusForSale(sales_id) {
    await $.ajax({
        ...
        success: function (result) {
            ...
            engineerStatusValues = Object.values(result.engineer_status_count).map(Number);
...
async function categoryStatusForAdmin() {
    await $.ajax({
        ...
        success: function (result) {
            ...
            adminCategoryStatusValues = Object.values(result.admin_category_status_count).map(Number);
        }
...
async function engineerStatusForAdmin() {
    await $.ajax({
        ...
        success: function (result) {
            ...
            adminEngineerStatusValues = Object.values(result.admin_engineer_status_count).map(Number);
        }
    });
...
async function typistStatusForAdmin() {
    await $.ajax({
        ...
        success: function (result) {
            ...
            adminTypistStatusValues = Object.values(result.admin_typist_status_count).map(Number);
        }
    });
...
async function regionForAdmin() {
    await $.ajax({
        ...
        success: function (result) {
            ...
            adminRegionValues = Object.values(result.admin_region_count).map(Number);
        }
    });
