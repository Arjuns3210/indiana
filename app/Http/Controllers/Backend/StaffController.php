...
class StaffController extends Controller
{
    ...
    {
        $data['data'] = Admin::with('region')->find($id);
        $data['country'] = Country::all();
    ...
    public function saveStaff(Request $request)
    {
        ...
        $expiryDate = $newDate->format('d-m-Y');
        ...
        if (isset($_GET['id'])) {
            ...
            $admins = new Admin;
            $admins->force_pwd_change_flag = 1;
        ...
        $admins->is_head = $request->is_head ? '1': '0';
        ...
        if (!empty($request->region)) {
            ...
            } else if (is_string($request->region)) {
                $region = $request->region;
            }
            $admins->region_id = $region;
        }
        if ($isEditFlow) {
